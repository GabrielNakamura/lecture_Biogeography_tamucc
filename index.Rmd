...
???
Thank you everyone for the presence. Today I pretend to talk with you a little about, community ecology,
...
???
Yes, photographs. This is not a art class, you are in the right class, but is interesting how photographs have some similarities with the way we try to understand the world around us thorough the lens of science. Photographs are an interesting thing since we can get some information based only on limited time-slices. Some people say that a good photograph is the one in which we can wonder what happened one second ago and whats gonna happen one second later from the shot we are looking at.
...
## Community Phylogenetic + Large scale patterns = More comprehensive view of nature?

???
So, if we use metrics that incorporate the evolutionary history of lineages in large scale schemes of regionalization, are we gonna be able to understand the role historical processes in assemblages and regions? 
The answer is no, and this next example illustrate why not
...
## Further possibilities: Application in regionalization schemes

.midle[
.center[
.pull-left[

This same approach can be applied to understand the role of dispersion and different regions in regionalization schemes
...
???
Regionalization schemes can also be improved by merging ancestral area reconstruction with current approaches of regionalization

.pull-right[
```{r echo=FALSE, out.width="100%", fig.alt= "A painting illustrating Darwin's finches" }
knitr::include_graphics("images/disp.from.png")
```
